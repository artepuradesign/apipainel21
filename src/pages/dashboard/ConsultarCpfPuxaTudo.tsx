...
const ConsultarCpfPuxaTudo: React.FC<ConsultarCpfPuxaTudoProps> = ({
  ...
  const [verificationPhase, setVerificationPhase] = useState<'server' | 'initial' | 'retry' | null>(null);
  ...
  const performSearch = async () => {
    ...
    try {
      ...
      if (!firstCheckResult.success || !firstCheckResult.data) {
        ...
        // ENVIAR PARA ATITO e acompanhar status real antes de notificar sucesso ao usuário
        ...
        console.log('🕐 [ATITO] CPF aceito no servidor. Aguardando confirmação real de envio ao Telegram...');
        setVerificationPhase('server');
        toast.info('Consulta recebida e em processamento no servidor', { duration: 2500 });
        ...
        while (Date.now() - statusStart < statusTimeoutMs) {
          const statusResult = await atitoConsultaCpfService.consultarStatus(cpf, atitoResult.statusUrl);

          if (!statusResult.success) {
            console.warn('⚠️ [ATITO_STATUS] Falha ao consultar status:', statusResult.error);
            await new Promise((resolve) => setTimeout(resolve, 1500));
            continue;
          }

          const currentStatus = statusResult.status;
          console.log('📡 [ATITO_STATUS] Status atual:', currentStatus, '-', statusResult.message);

          if (currentStatus === 'error' || currentStatus === 'timeout') {
            toast.error(statusResult.message || 'Falha no processamento externo', { duration: 3000 });
            setVerificationLoadingOpen(false);
            setLoading(false);
            return;
          }

          if (currentStatus === 'telegram_sent' || currentStatus === 'n8n_sent') {
            telegramConfirmed = true;
            n8nConfirmed = currentStatus === 'n8n_sent';
            break;
          }
        ...
        if (!telegramConfirmed) {
          toast.error('Não foi possível confirmar envio ao Telegram dentro do tempo esperado', { duration: 3500 });
          setVerificationLoadingOpen(false);
          setLoading(false);
          return;
        }

        toast.success('CPF enviado ao Telegram com sucesso', { duration: 2000 });
        ...
        const tickCountdown = async (secondsTotal: number) => {
          for (let elapsed = 0; elapsed < secondsTotal; elapsed++) {
            ...
            // Aceleração: se n8n já confirmou antes do fim da contagem, podemos encurtar espera
            if (!n8nConfirmed) {
              const statusResult = await atitoConsultaCpfService.consultarStatus(cpf, atitoResult.statusUrl);
              if (statusResult.success && statusResult.status === 'n8n_sent') {
                n8nConfirmed = true;
                console.log('⚡ [ATITO_STATUS] n8n confirmado durante contagem, acelerando verificação de banco.');
                break;
              }
            }
          }
                ...
                <DialogHeader>
                  ...
                  <DialogDescription className="text-center text-sm sm:text-base">
                    {verificationPhase === 'retry'
                      ...
                      : verificationPhase === 'server'
                        ? 'Consulta recebida: aguardando confirmação de envio ao Telegram...'
